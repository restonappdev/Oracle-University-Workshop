...
## Introduction
This lab will dive into the details of development of the bot. We will learn about Intent & Entity feature of Bots, create instant app for backend login module, set up Backends & APIs  for bot backend service and lastly add dialog flows for Service which integrates the get or update information, this will make Gen University Bot more interactive.

## Objectives
- Learn about Intent & Entities feature of AMCes
- Add an instant app of backend login module to the bot interaction
- Set up Backend & APIs for backend service
...
- While intents map words and phrases to a specific action, entities add context to the intent itself. They help to describe the intent more fully and enable your bot to complete a user request. The Location intent, for example, describes a user request, but only in general terms. To fill in the specifics, this intent is augmented by the Facilities entity, which identifies values like 'morill Hall' from the user input.

![](./images/200/2.png)

### Step 2: Add an instant app of backend login module to the bot interaction
...
### Step 3: Set up Backend & APIs for backend service
